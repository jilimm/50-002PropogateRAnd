/*
   This file was generated automatically by the Mojo IDE version B1.3.6.
   Do not edit this file directly. Instead edit the original Lucid source.
   This is a temporary file and any changes made to it will be destroyed.
*/

module mojo_top_0 (
    input clk,
    input rst,
    input cclk,
    output reg spi_miso,
    input spi_ss,
    input spi_mosi,
    input spi_sck,
    output reg [3:0] spi_channel,
    input avr_tx,
    output reg avr_rx,
    input avr_rx_busy,
    output reg low1,
    output reg low2,
    output reg high1,
    output reg high2,
    output reg high3,
    output reg low3,
    output reg [7:0] led,
    output reg [23:0] io_led
  );
  
  
  
  wire [2-1:0] M_mypropogater_rowLit;
  wire [3-1:0] M_mypropogater_numOut;
  wire [1-1:0] M_mypropogater_row1;
  wire [1-1:0] M_mypropogater_row2;
  wire [1-1:0] M_mypropogater_row3;
  reg [3-1:0] M_mypropogater_num;
  propogate_1 mypropogater (
    .clk(clk),
    .rst(rst),
    .num(M_mypropogater_num),
    .rowLit(M_mypropogater_rowLit),
    .numOut(M_mypropogater_numOut),
    .row1(M_mypropogater_row1),
    .row2(M_mypropogater_row2),
    .row3(M_mypropogater_row3)
  );
  wire [3-1:0] M_myrandonmizer_num;
  reg [2-1:0] M_myrandonmizer_rowNum;
  randomize_2 myrandonmizer (
    .clk(clk),
    .rst(rst),
    .rowNum(M_myrandonmizer_rowNum),
    .num(M_myrandonmizer_num)
  );
  
  always @* begin
    spi_miso = 1'bz;
    spi_channel = 4'bzzzz;
    avr_rx = 1'bz;
    M_mypropogater_num = M_myrandonmizer_num;
    M_myrandonmizer_rowNum = M_mypropogater_rowLit;
    high1 = M_mypropogater_row1;
    high2 = M_mypropogater_row2;
    high3 = M_mypropogater_row3;
    low1 = M_mypropogater_numOut[0+0-:1];
    low2 = M_mypropogater_numOut[1+0-:1];
    low3 = M_mypropogater_numOut[2+0-:1];
    led[0+2-:3] = M_myrandonmizer_num;
    led[3+4-:5] = 5'h00;
    io_led[0+0+0-:1] = M_mypropogater_row1;
    io_led[0+1+0-:1] = M_mypropogater_row2;
    io_led[0+2+0-:1] = M_mypropogater_row3;
  end
endmodule
